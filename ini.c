...
#ifndef _WINDOWS
{
  ...
  strcpy(fullpathname,digger_dir);
...
#else
{
  ...
  strcpy(fullpathname,digger_dir);
  strcat(fullpathname,filename);
  WritePrivateProfileString(section,key,value,fullpathname);
}
#endif
                  ...
                  int destsize,char* filename)
{
  ...
  int i;
#ifdef __OS2__
// Windows version need this block too... I think...
  char fullpathname[255];
  ...
  do {
    fgets(s1,80,fp);
    sgets(s1,s1);
    if (stricmp(s1,sectstr)==0) {
      do {
        fgets(s1,80,fp);
        sgets(s1,s1);
        if (strnicmp(s1,keystr,strlen(keystr))==0) {
